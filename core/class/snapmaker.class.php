...
class snapmaker extends eqLogic {
  ...
  */
  ...
  public function toHtml($_version = 'dashboard') {
    ...
    $replace['#heightfilelist#'] = strval(intval($replace['#height#'])-300);
    ...
    switch ($this->getLogicalId()) { //vérifie le logicalid de la commande
      case 'refresh':
        ...
        foreach ($files as $file) {
          ...
          $filelist[] = array(
            'name' => $file,
            'size' => convert(filesize($filedir)),
            'date' => date("Y-m-d H:i:s", filemtime($filedir)),
            'type' => $header_type,
        ...
        log::add('snapmaker', 'debug', 'filelist : ' . json_encode($filelist));
